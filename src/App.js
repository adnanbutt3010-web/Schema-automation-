...
(function() {
  function extractPrice(title) {
    // Multi-currency support: PKR, USD, AED, INR, GBP, EUR, SAR, etc.
    var patterns = [
      // Symbol/code BEFORE number — "- Rs.850", "- $99", "- AED 250", "- ₹500", "- USD 99.99"
      /[-–]\\s*(Rs\\.?|PKR|USD|US\\$|AED|INR|GBP|EUR|SAR|AUD|CAD|د\\.إ|₹|€|£|\\$)\\s*([\\d,]+(?:\\.\\d{1,2})?)/i,
      // Number BEFORE symbol/code — "- 850 Rs", "- 99 USD", "- 250 AED"
      /[-–]\\s*([\\d,]+(?:\\.\\d{1,2})?)\\s*(Rs\\.?|PKR|USD|AED|INR|GBP|EUR|SAR|AUD|CAD)/i,
      // "Price: Rs.850" / "Price: $99"
      /Price[:\\s]+(Rs\\.?|PKR|USD|AED|INR|GBP|EUR|SAR|\\$|₹|€|£)?\\s*([\\d,]+(?:\\.\\d{1,2})?)/i,
      // Fallback anywhere in title
      /(Rs\\.?|PKR|USD|AED|INR|GBP|EUR|SAR|\\$|₹|€|£)\\s*([\\d,]+(?:\\.\\d{1,2})?)/i,
      /([\\d,]+(?:\\.\\d{1,2})?)\\s*(Rs\\.?|PKR|USD|AED|INR|GBP|EUR|SAR)/i,
    ];

    var symToCurrency = {
      'rs':'PKR','rs.':'PKR','pkr':'PKR','₹':'INR','inr':'INR',
      '$':'USD','us$':'USD','usd':'USD','aed':'AED','د.إ':'AED',
      '£':'GBP','gbp':'GBP','€':'EUR','eur':'EUR','sar':'SAR',
      'aud':'AUD','cad':'CAD'
    };
    ...
    for (var i = 0; i < patterns.length; i++) {
      ...
      if (match) {
        var g1 = match[1], g2 = match[2];
        var amount, symbol;
        if (g1 && /[\\d]/.test(g1)) { amount = g1; symbol = g2; }
        else { symbol = g1; amount = g2; }
        if (!amount) continue;
        var detectedCurrency = symbol ? (symToCurrency[symbol.toLowerCase()] || '${cur}') : '${cur}';
        return { price: amount.replace(/,/g, ''), currency: detectedCurrency };
      }
    }
    return null;
  }
  ...
  function injectProductSchema() {
    ...
    var result = extractPrice(postTitle) || extractPrice(title);

    if (!result) return; // Price nahi mili toh schema inject mat karo
    var price = result.price;
    var detectedCurrency = result.currency;
    ...
    var schema = {
      ...
      "@type": "Product",
      "name": postTitle.replace(/\\s*[-–]\\s*.*$/i, '').trim(),
      "description": desc,
      "image": img,
      "url": url,
              ...
              <div className="fade">
                ...
                {productS?.enabled&&productS?.fields?.bloggerAutoPrice&&(
                  <div className="highlight" style={{marginTop:20}}>
                    <div style={{fontWeight:700,fontSize:14,color:"#92400e",marginBottom:6}}>💡 Blogger Post Title Format — Koi Bhi Currency:</div>
                    <div style={{fontFamily:"monospace",fontSize:12.5,color:"#1e1b4b",marginBottom:4,lineHeight:2}}>
                      <strong>Hard Wax Beans 100g - Rs.850</strong> (Pakistan)<br/>
                      <strong>Wireless Mouse - AED 99</strong> (UAE)<br/>
                      <strong>Phone Case - $19.99</strong> (USA)<br/>
                      <strong>T-Shirt - £15</strong> (UK)
                    </div>
                    <div style={{fontSize:12,color:"#78350f"}}>
                      Title mein <strong>"- [Currency][Price]"</strong> likhو — JS automatically currency + price detect kar lega! Rs, PKR, $, USD, AED, £, GBP, €, EUR, ₹, INR, SAR sab support hain ✅
                    </div>
                  </div>
                )}
              </div>
